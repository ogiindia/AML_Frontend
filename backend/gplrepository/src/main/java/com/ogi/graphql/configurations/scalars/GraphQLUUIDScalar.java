...
public class GraphQLUUIDScalar {
	...
	static {
		Coercing<UUID, String> coercing = new Coercing<UUID, String>() {
			...
			public String serialize(Object input) throws CoercingSerializeException {
				...
				if (result == null) {
					throw new CoercingSerializeException("Expected type 'UUID' but was '" + (input) + "'.");
				}
			...
			public UUID parseValue(Object input) throws CoercingParseValueException {
				UUID result = convertImpl(input);
				if (result == null) {
					throw new CoercingParseValueException("Expected type 'UUID' but was '" + (input) + "'.");
				}
				return result;
			}
			...
			public UUID parseLiteral(Object input) throws CoercingParseLiteralException {
				if (!(input instanceof StringValue)) {
					throw new CoercingParseLiteralException(
							"Expected a 'java.util.UUID' AST type object but was '" + input + "'.");
				}
				try {
		...
		INSTANCE = GraphQLScalarType.newScalar().name("UUID")
				.description("A universally unique identifier compliant UUID Scalar").coercing(coercing).build();
